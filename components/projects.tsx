...
const PROJECT_ACCENTS: Record<string, { accent: string; muted: string; glow: string }> = {
  kasuwa:     { accent: '#8FA58D', muted: 'rgba(143, 165, 141, 0.12)', glow: 'rgba(143, 165, 141, 0.18)' },
  liora:      { accent: '#9B8FBC', muted: 'rgba(155, 143, 188, 0.12)', glow: 'rgba(155, 143, 188, 0.18)' },
  tradingpal: { accent: '#7187C4', muted: 'rgba(113, 135, 196, 0.12)', glow: 'rgba(113, 135, 196, 0.18)' },
  bloomstudio:{ accent: '#C4A66B', muted: 'rgba(196, 166, 107, 0.12)', glow: 'rgba(196, 166, 107, 0.18)' },
  medconnect: { accent: '#729B98', muted: 'rgba(114, 155, 152, 0.12)', glow: 'rgba(114, 155, 152, 0.18)' },
};

const DEFAULT_ACCENT = { accent: '#C98B7C', muted: 'rgba(201, 139, 124, 0.12)', glow: 'rgba(201, 139, 124, 0.18)' };
  ...
  return (
    <section id="work" className="section-spacing bg-muted/30">
      <div className="section-container section-padding">
        ...
        <div ref={ref} className="space-y-6 md:space-y-10">
          {projects.map((project: any, i: number) => {
            const uploadedImage = images?.[project.id] || project.image || "";
            const projectAccent = PROJECT_ACCENTS[project.id] || DEFAULT_ACCENT;
            return (
            <div
              key={project.id}
              className={cn(
            ...
            >
              ...
              >
                {/* Project image — CLEAN, no browser bar */}
                <div
                  className={cn(
                    "relative",
                    i % 2 !== 0 && "md:order-2"
                ...
                >
                  <div className="rounded-xl border border-border bg-surface shadow-xl shadow-black/[0.04] overflow-hidden transition-all duration-500 group-hover:shadow-2xl group-hover:shadow-black/[0.06] group-hover:-translate-y-1">
                    ...
                    >
                      {uploadedImage ? (
                        ...
                        /* Clean fallback — no browser bars, just stylized placeholder */
                  ...
                  <div
                    ...
                    style={{ background: `radial-gradient(ellipse at center, ${projectAccent.glow} 0%, transparent 70%)` }}
                  />
                </div>
                ...
                >
                  <span
                    ...
                    style={{ color: `color-mix(in srgb, ${projectAccent.accent} 45%, #78716c)` }}
                  >
                    {project.category}
                  </span>
                  ...
                  {project.url && (
                    <a
                      ...
                      style={{ color: `color-mix(in srgb, ${projectAccent.accent} 60%, var(--color-primary))` }}
                    >
                      View Project
